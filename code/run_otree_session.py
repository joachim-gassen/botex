...
def setup_bot_db():
    ...
    if not table_exists:
        cursor.execute("CREATE TABLE sessions (session_id char(8), participant_id char(8), is_human integer, url text)")
        conn.commit()
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='conversations'")
    table_exists = cursor.fetchone()
    if not table_exists:
        cursor.execute("CREATE TABLE conversations (id char(8), conversation text)")
        conn.commit()
    cursor.close()
    return conn
...
if nbots > PLAYERS: raise(Exception(f"Wrong entry - bot number cannot be higher than {PLAYERS}."))
bots_urls = [urls[i] for i in sample(range(PLAYERS), nbots)]
human_urls = list(set(urls) - set(bots_urls))

rows = zip([session_id]*PLAYERS, part_codes, [x in human_urls for x in urls], urls)

conn = setup_bot_db()
cursor = conn.cursor()
cursor.executemany("INSERT INTO sessions (session_id, participant_id, is_human, url) VALUES (?, ?, ?, ?)", rows) 
conn.commit()
cursor.close()
...
for url in bots_urls:
    Thread(target = run_bot, args=(url,)).start()
